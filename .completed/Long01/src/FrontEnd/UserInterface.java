...
public class UserInterface {
    ...
    private void selectionOne() {
        ...
        while (numberofStudents < minUserInput) {
            try {
                ...
                numberofStudents = Integer.parseInt(this.enterAValue(
                        "Please enter number of student you want to add: ", 
                        countValidation, 
                        String.format("Must be a number (>= %d)", this.minUserInput), 
                        true));

                // If user enters a number of student lower than the required number
                if (numberofStudents < minUserInput) {
                    System.out.println(String.format("Must be a number (>= %d)", this.minUserInput));
                }
    ...
    /**
     * Selection 6 - Add new course to existing student
     */
    private void selectionSix() {
        // Ask user to enter id.
        String studentId = this.enterAValue("Enter student Id: ", studentIdValidation, "Must be a number (1 to 10 digits)", true);
        while (!studentDb.isIdExisted(studentId)) {
            System.out.println("That id does not exist. Please re-enter");
            studentId = this.enterAValue("Enter student Id: ", studentIdValidation, "Must be a number (1 to 10 digits)", true);
        }
        
        // Get student name.
        // If string is empty, student id just 
        String studentName = studentDb.retNameFromId(studentId);
        System.out.println("Registering new course for student: " + studentName);
        
        String semester = this.enterAValue("Enter semester: ", semesterValidation, "Must be a number (1 to 2 digits)", true);

        String courseName = this.enterAValue("Enter course name: ", courseValidation, "Either \"Java\" or \".Net\" or \"C/C++\" (case sensitive)", true);

        while (studentDb.isIDAndSemesterExisted(studentId, semester, courseName)) {
            System.out.println("A duplicated course is found.");
            courseName = this.enterAValue("Enter course name: ", courseValidation, "Either \"Java\" or \".Net\" or \"C/C++\" (case sensitive)", true);
        }
        ...
        while (true) {
            try {
                System.out.print(
                        ...
                        + " 4. Report\n"
                        + " 5. Exit\n"
                        + " 6. Add course to existing student\n"
                        + "Please choose: "
                );
